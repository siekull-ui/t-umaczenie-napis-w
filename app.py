...
def apply_hero_layout():
    ...
    .stApp { background-color: #F0D3DE !important; }
    ...
    .hero-nav label:hover { color: #FF2A5F; }

    .hero-nav label::after {
        content: ''; position: absolute;
        bottom: -10px; left: -15px; right: -15px; height: 6px;
        ...
        background-repeat: repeat-x; opacity: 0;
        transition: opacity 0.3s ease;
    }

    .search-icon {
        margin-left: -20px; display: flex; align-items: center; cursor: pointer;
    }
    .search-icon svg { width: 18px; height: 18px; stroke: #1a1a1a; stroke-width: 2; transition: stroke 0.2s ease; }
    .search-icon:hover svg { stroke: #FF2A5F; }
    .search-icon::after { display: none !important; }

    /* UKRYTE INPUTY */
    input[name="hero-tabs"] { display: none; }
    ...
    # HTML zamknięty w zmiennej (z brakującymi inputami!)
    html_structure = """
    <div id="hero-canvas">
